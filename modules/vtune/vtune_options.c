...
static int parse_env_vars(void) {
    if (NULL != getenv("PERFEXPERT_VTUNE_PREFIX")) {
        arg_options.prefix = ("PERFEXPERT_VTUNE_PREFIX");
        OUTPUT_VERBOSE((1, "ENV: prefix=%s", arg_options.prefix));
    }

    if (NULL != getenv("PERFEXPERT_VTUNE_BEFORE")) {
        arg_options.before = ("PERFEXPERT_VTUNE_BEFORE");
        OUTPUT_VERBOSE((1, "ENV: before=%s", arg_options.before));
    }

    if (NULL != getenv("PERFEXPERT_VTUNE_AFTER")) {
        arg_options.after = ("PERFEXPERT_VTUNE_AFTER");
        OUTPUT_VERBOSE((1, "ENV: after=%s", arg_options.after));
    }

    if (NULL != getenv("PERFEXPERT_VTUNE_MIC_CARD")) {
        my_module_globals.mic = ("PERFEXPERT_VTUNE_MIC_CARD");
        OUTPUT_VERBOSE((1, "ENV: mic=%s", my_module_globals.mic));
    }

    if (NULL != getenv("PERFEXPERT_VTUNE_MIC_PREFIX")) {
        arg_options.mic_prefix = ("PERFEXPERT_VTUNE_MIC_PREFIX");
        OUTPUT_VERBOSE((1, "ENV: mic_prefix=%s", arg_options.mic_prefix));
    }

    if (NULL != getenv("PERFEXPERT_VTUNE_MIC_BEFORE")) {
        arg_options.mic_before = ("PERFEXPERT_VTUNE_MIC_BEFORE");
        OUTPUT_VERBOSE((1, "ENV: mic_before=%s", arg_options.mic_before));
    }

    if (NULL != getenv("PERFEXPERT_VTUNE_MIC_AFTER")) {
        arg_options.mic_after = ("PERFEXPERT_VTUNE_MIC_AFTER");
