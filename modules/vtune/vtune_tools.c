...
int parse_report (const char * parse_file, vtune_hw_profile_t *profile) {
    ...
    int i;
    char *events[MAX_ARGUMENTS_COUNT];
    int argc;
    ...
    if (PERFEXPERT_SUCCESS!=parse_line (header, events, &argc)) {
    ...
    while (fgets(line,MAX_BUFFER_SIZE, in)) {
        ...
        OUTPUT_VERBOSE ((8, "%s %s", "parsing line", line));
...
//    char res_folder [MAX_FILENAME];
    ...
    strcpy (my_module_globals.res_folder, template);
