...
int create_report (char* results_folder, const char* parse_file) {
    ...
    argv[argc] = "-r";
    argc++;
    argv[argc] = my_module_globals.res_folder; //results_folder;
    argc++;
...
int parse_report (const char * parse_file, vtune_hw_profile_t *profile) {
    ...
    while (fgets(line,MAX_BUFFER_SIZE, in)) {
        ...
        OUTPUT_VERBOSE ((8, "processing line %s", line));
        if (PERFEXPERT_SUCCESS!=parse_line (line, argv, &argc)) {
            OUTPUT (("%s", _ERROR ("unable to parse the report generated by VTune")));
            fclose(in);
            return PERFEXPERT_ERROR;
        }
        OUTPUT_VERBOSE ((8, "line processed"));
...
int run_amplxe_cl(void) {
    ...
    OUTPUT_VERBOSE ((8, "initializing res folder"));
    sprintf(my_module_globals.res_folder, "%s/%s", globals.moduledir, template);
//    strcpy (my_module_globals.res_folder, globals.moduledir);
//    strcat (my_module_globals.res_folder, template);
    mktemp(my_module_globals.res_folder);
    OUTPUT_VERBOSE ((8, "res folder initialized %s", my_module_globals.res_folder));
    ...
    argv[argc] = "-r";
    argc++;
    argv[argc] = my_module_globals.res_folder;//res_folder;
    argc++;
    // argv[argc] = "runsa-knc";
    // argc++;
    argv[argc] = "-knob";
...
int collect_results(vtune_hw_profile_t *profile) {
    ...
    if (PERFEXPERT_SUCCESS!=create_report ("", csv_name)) {
        OUTPUT (("%s", _ERROR ("unable to create a report file from the results generated by VTune")));
