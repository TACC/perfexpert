...
#include "install_dirs.h"
 ...
 //           else {
                ...
                //            itoa(papi_get_sampling_rate(event->name), samples, 10);
...
PERFEXPERT_ALLOC(char, script_file, (strlen(globals.moduledir) + 20));
sprintf(script_file, "%s/mic_hpcrun-%d.sh", globals.moduledir, experiment_count);
...
OUTPUT_VERBOSE((10, "EVENT COUNT %d", event_count));
event_count--;
continue;
}
