...
static int parse_env_vars(void) {
    if (NULL != getenv("PERFEXPERT_MODULE_ICC_SOURCE")) {
        my_module_globals.source = getenv("PERFEXPERT_MODULE_ICC_SOURCE");
        OUTPUT_VERBOSE((1, "ENV: source=%f", my_module_globals.source));
    }

    if (NULL != getenv("PERFEXPERT_MODULE_ICC_FULL_PATH")) {
        my_module_globals.compiler = getenv("PERFEXPERT_MODULE_ICC_FULL_PATH");
