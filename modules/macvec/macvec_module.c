...
#include "common/perfexpert_string.h"
    ...
    int module_init(void) {
        ...
        if ((PERFEXPERT_FALSE == perfexpert_module_available("hpctoolkit")) &&
            ...
            OUTPUT(("%s",
                        _RED("PerfExpert will try to load HPCToolkit module")));
        }
        ...
        if (PERFEXPERT_TRUE == perfexpert_module_available("vtune")) {
            ...
            if (PERFEXPERT_SUCCESS != perfexpert_module_requires("macvec",
                        PERFEXPERT_PHASE_ANALYZE, "vtune",
                        PERFEXPERT_PHASE_MEASURE, PERFEXPERT_MODULE_BEFORE)) {
                OUTPUT(("%s", _ERROR("required module/phase not available")));
                return PERFEXPERT_ERROR;
            }
        ...
        if (NULL == my_module_globals.architecture) {
            ...
            sprintf(sql, "SELECT description FROM lcpi_proc WHERE family=%d "
            ...
            if (NULL == my_module_globals.architecture) {
                OUTPUT_VERBOSE((1,
                            "Unknown architecture, using PAPI defaults"));
        ...
        char* architecture = my_module_globals.architecture;

        /* Jaketown (or SandyBridgeEP) */
        if (0 == strcmp("jaketown", 
                    perfexpert_string_to_lower(architecture))) {
        ...
        /* If not any of the above, I'm sorry... */
            ...
            OUTPUT(("%s (%s)", _ERROR("setting counters"), architecture));
            return PERFEXPERT_ERROR;
        }
    ...
    int module_analyze(void) {
        ...
        /* For each profile... */
        perfexpert_list_for(p, &(my_module_globals.profiles),
                macvec_profile_t) {
