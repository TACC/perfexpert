...
double pHit(unsigned long i, unsigned long assoc, unsigned long distance, double pHitSet)
{
...
double hit_probability(long distance, long size, short associativity, int linesize)
{
	long numsets = (long) (size / ((double) (associativity * linesize)));
	double pHitSet=1.0/numsets, p=0;
	long i, target=MIN(associativity-1, distance);
...
int main()
{
	printf ("Result = %lf\n", hit_probability(174*64, 256*1024, 8, 64));
	return 0;
}
#endif
