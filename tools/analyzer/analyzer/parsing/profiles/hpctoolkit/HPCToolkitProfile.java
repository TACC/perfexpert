...
public class HPCToolkitProfile extends AProfile {
    ...
    public double getMetricBasedOnPEIndex(int PEIndex) {
        if ((0 == counts[PEIndex]) || (0 == perfValues[PEIndex])) {
            return 0;
        }
    ...
    private void setMetricBasedOnPEIndex(int PEIndex, double value) {
        ...
        if (PEIndex == profileConstants.indexOfCycles) {
            // Set importance
            if (0 != profileConstants.aggregateCycles) {
                importance = (perfValues[PEIndex] / counts[PEIndex]) /
                             ((double) profileConstants.aggregateCycles);
            } else {
    ...
    public double getVariation() {
        if (maxInstructions == Double.MIN_VALUE) {
            // Value was never set
            return 0;
        }
    ...
    private double roundToTwoDigitsAfterDecimal(double in) {
        try {
            ...
            log.error("Failed to parse while rounding \"" + in +
                      "\" to two digits, defaulting to zero");
            return 0;
        }
    }
}
