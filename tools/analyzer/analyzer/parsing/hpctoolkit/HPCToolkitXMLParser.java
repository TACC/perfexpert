...
public class HPCToolkitXMLParser extends DefaultHandler {
    ...
    public void startElement(String uri, String localName, String qName,
                             Attributes attr) throws XMLParsingDoneException {
        // Everything to be ignored till we see a ending "C" element
        if (inCallSite) {
            log.debug("Skipping element \"" + qName +
                      "\" because we are currently within a callsite (C) element");
            return;
        }
        ...
        if (qName.equals("P")) {
            ...
            log.debug("Found a \"P\" element for \"" + procedureName + "\"" +
                      codeSection + ", starting to record a new Profile object");

            profile = new HPCToolkitProfile(profileConstants);
            profile.setCodeSectionInfo("Function " + procedureName +
                                       (procedureName.contains("(") ? "" : "()") +
                                       codeSection);
            
            // Fialho: adding some info to make my life easier.
            profile.setCodeProcedureName(procedureName);
        ...
        if (qName.equals("L")) {
            ...
            loopDepth++;
            log.debug("Found an \"L\" element for" + codeSection +
                      ", starting to record a new Profile object");
        ...
        if (qName.equals("M")) {
            if ((true == aggregateRecorded) && (null == profile)) {
                log.debug("Found an \"M\" element but ignored, looks like it was under an LM, S, or F");
                // Ignore because this metric must be under an LM or S or F
                return;
            }

            log.debug("Found an \"M\" element");
            if ((false == aggregateRecorded) && (null != profile)) {
        ...
        if (qName.equals("Metric")) {
            ...
            if (m.find()) {
                ...
                log.debug("Encountered \"Metric\" element that did not match the regex: " +
    ...
    public void endElement(String uri, String localName,
                           String qName) throws XMLParsingDoneException {
        ...
        // If it is a procedure or a loop that has ended, reset the Profile
        // object so that we don't (even by mistake) incorrectly record data
        if (qName.equals("P")) {
    ...
    void setAggregateCyclesFromRootProfile() {
        ...
        if (0 == profileConstants.getAggregateCycles()) {
        ...
        if (-1 == lastProfile.getImportance()) {
            // Skip this one for now because its importance has not yet been calculated
            return;
        }

        if (lastProfile.getImportance() < threshold) {
            log.debug("Removing profile \"" + lastProfile.getCodeSectionInfo() +
                      "\" because its importance is " +
                      lastProfile.getImportance() + " and threshold is " +
                      threshold);
    ...
    String formatCodeSection(int lineNumber, boolean newProcedure) {
        ...
        if ((0 == lineNumber) && (null == procedureName) &&
            ((null == filename) || filename.equals("~unknown-file~"))) {
            location = "~unknown-location~";
        } else if ((null != filename) && !filename.equals("~unknown-file~") &&
                   (0 != lineNumber)) {
            location = ((false == newProcedure) ? ((null != procedureName) ? " in function " + procedureName + (procedureName.contains("(") ? "" : "()") : "") : "") + " at " + filename + ":" + lineNumber;
        } else {
            location = (0 != lineNumber) ? " line " + lineNumber : "" + ((false == newProcedure) ? ((null != procedureName) ? " in function " + procedureName + (procedureName.contains("(") ? "" : "()") : "") : "") + ((null != filename) && !filename.equals("~unknown-file~") ? " in file \"" + filename + "\"" : "");
        }
    ...
    String formatFilename(String filename, int lineNumber) {
        if (lineNumber == 0 && isFilenameEmpty(filename)) {
            return null;
        } else if ((0 != lineNumber) && isFilenameEmpty(filename)) {
    ...
    boolean isFilenameEmpty(String filename) {
        return null == filename || filename.equals("~unknown-file~");
    }
