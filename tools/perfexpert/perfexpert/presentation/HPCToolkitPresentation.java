...
public class HPCToolkitPresentation
{
	...
	public static void presentRecommendations(List<HPCToolkitProfile> profiles01, LCPIConfigManager lcpiConfig, MachineConfigManager machineConfig, String file01, boolean aggregateOnly, int maxSuggestions)
	{
		...
		for (HPCToolkitProfile profile : profiles01)
		{
			...
			if (machineConfig.getProperties().getProperty("good-int-CPI") != null)
				lcpi.put("good-int-CPI", Float.parseFloat(machineConfig.getProperties().getProperty("good-int-CPI")));

			if (machineConfig.getProperties().getProperty("good-fp-CPI") != null)
				lcpi.put("good-fp-CPI", Float.parseFloat(machineConfig.getProperties().getProperty("good-fp-CPI")));

            // Fialho: removing the AutoSCOPE functionality from PerfExpert, outputing measurement when --recommend is in use.
			//AutoSCOPE as = new AutoSCOPE("/optimizations/opt-sug-intel.pdb");
			//as.addCodeSection(profile.getCodeSectionInfo() + " (" + doubleFormat.format(profile.getImportance()*100) + "% of the total runtime)", lcpi);
            System.out.println("%");
            ...
            Set s = lcpi.entrySet(); // PerfExpert metrics with correspondent measurements
            Iterator it = s.iterator();
            while (it.hasNext()) {
                Map.Entry m = (Map.Entry)it.next();
                System.out.println("perfexpert." + m.getKey() + "=" + m.getValue());
            }
            s = perfCtrTranslation.entrySet(); // PAPI raw measurements
            it = s.iterator();
            while (it.hasNext()) {
                Map.Entry m = (Map.Entry)it.next();
                System.out.println("papi." + m.getKey() + "=" + m.getValue());
            }
            s = machineConfig.getProperties().entrySet(); // Hound host measurements
            it = s.iterator();
            while (it.hasNext()) {
                Map.Entry m = (Map.Entry)it.next();
