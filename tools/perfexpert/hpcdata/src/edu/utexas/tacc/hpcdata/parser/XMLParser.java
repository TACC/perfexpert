...
public class XMLParser extends DefaultHandler
{
	...
	public void startElement(String uri, String localName, String qName, Attributes attr) throws SAXException
	{
		...
		if (qName.equals("LoadModule"))
			if (attr.getValue("i") != null && attr.getValue("n") != null)
				module_name_map.put(Long.parseLong(attr.getValue("i")), attr.getValue("n").replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;"));

		if (qName.equals("File"))
			if (attr.getValue("i") != null && attr.getValue("n") != null)
			{
				String [] file_bits = attr.getValue("n").split("/");
				file_name_map.put(Long.parseLong(attr.getValue("i")), file_bits[file_bits.length-1].replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;"));
			}
		...
		if (qName.equals("Procedure"))
		{
			...
			if (attr.getValue("i") != null && attr.getValue("n") != null)
				proc_name_map.put(Long.parseLong(attr.getValue("i")), attr.getValue("n").replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;"));
		}

		if (qName.equals("PF") || qName.equals("Pr"))
		{
			if (attr.getValue("n") == null)
			{
			...
			if (!codeSegmentStack.empty())
			{
				...
				if (topNode != null)	topNode.addChild(p);
			}
		...
		if (qName.equals("M"))
		{
			if (attr.getValue("n") == null || attr.getValue("v") == null)
			{
	...
	void percolateUp(Node parent, long n, double v)
	{
		...
		{
			if (((Procedure) parent).getA() == 1)	// Inlined (alien)
			{
	...
	public void writeResults(PrintStream out)
	{
		...
		for (i=0; i<metric_name_list.size(); i++)
		{
			...
			if (val != 0)	Printer.putM(i, aggregate_metrics.get((int) i));
		}

		if (procedureSeq.size() != 0)
		{
		...
		if (n.getType().equals("P"))
		{
			...
			else
			{
				if (p.getA() != 1)
				{
					...
					file = p.getF();
					Printer.putP(p.getN(), proc_name_map.get(p.getN()), p.getL());
				}
				else
					Printer.putP(p.getN(), "inlined from " + file_name_map.get(p.getF()) + ": " + p.getL(), p.getL());
