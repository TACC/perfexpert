...
public class PerfExpert
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		FlaggedOption flRecommend = new FlaggedOption('r', "recommend", "Recommend suggestions for optimization", "limit", false, ArgType.INTEGER);
		FlaggedOption flThreads = new FlaggedOption('t', "threads", "Show information for specific threads", "thread#", true, ArgType.STRING);
		UnflaggedOption ufThreshold = new UnflaggedOption("threshold", "Threshold between 0 and 1", ArgType.DOUBLE);
		UnflaggedOption ufExp01 = new UnflaggedOption("experiment.xml", "experiment.xml file generated using `perfexpert_run_exp'", ArgType.STRING);
		UnflaggedOption ufExp02 = new UnflaggedOption("experiment.xml", "Second experiment.xml file, for comparison only. Not valid with -r,--recommend", ArgType.STRING);
		...
		// Sanity check
		if ((parser.isSet(flRecommend) && parser.isSet(ufExp02)) || parser.isSet(swHelp))
		{
			printHelp(parser);
			return;
		}
		...
		if (recommend)	max_suggestions=(Integer) parser.getValue(flRecommend);
		if (max_suggestions==null)	max_suggestions=0;
		...
		if (!recommend)
			...
			HPCToolkitPresentation.presentRecommendations(profiles01, lcpiConfig, machineConfig, filename01, aggregateOnly, max_suggestions);
	}
}
