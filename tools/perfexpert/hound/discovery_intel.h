...
const intelCacheTableEntry intelCacheTable[CACHE_TABLE_SIZE] = {
    ...
    0x50, { TLB,   INSTRUCTION, DONT_CARE, PAGESIZE_4K|PAGESIZE_2M|PAGESIZE_4M,
                                                        64,     FULLY_ASSOCIATIVE },
    0x51, { TLB,   INSTRUCTION, DONT_CARE, PAGESIZE_4K|PAGESIZE_2M|PAGESIZE_4M,
                                                        128,    FULLY_ASSOCIATIVE },
    0x52, { TLB,   INSTRUCTION, DONT_CARE, PAGESIZE_4K|PAGESIZE_2M|PAGESIZE_4M,
                                                        256,    FULLY_ASSOCIATIVE },
    0x55, { TLB,   INSTRUCTION, DONT_CARE, PAGESIZE_2M|PAGESIZE_4M,
                                                        7,      FULLY_ASSOCIATIVE },
    0x56, { TLB,   DATA,        1,         PAGESIZE_4M, 16,     4 },
    0x57, { TLB,   DATA,        1,         PAGESIZE_4K, 16,     4 },
    0x5A, { TLB,   DATA,        DONT_CARE, PAGESIZE_2M|PAGESIZE_4M,
                                                        32,     4 },
    0x5B, { TLB,   DATA,        DONT_CARE, PAGESIZE_4K|PAGESIZE_4M,
                                                        64,     FULLY_ASSOCIATIVE },
    0x5C, { TLB,   DATA,        DONT_CARE, PAGESIZE_4K|PAGESIZE_4M,
                                                        128,    FULLY_ASSOCIATIVE },
    0x5D, { TLB,   DATA,        DONT_CARE, PAGESIZE_4K|PAGESIZE_4M,
                                                        256,    FULLY_ASSOCIATIVE },
...
int insertIntoCorrectCacheList(cacheCollection *lpCaches, cacheInfo cache) {
    ...
    if (1 == cache.level || DONT_CARE == cache.level) { // Insert into L1
        lplpCacheList = &lpCaches->lpL1Caches;
    } else if (2 == cache.level) { // Insert into L2
        lplpCacheList = &lpCaches->lpL2Caches;
    } else if (3 == cache.level) { // Insert into L3
...
int mapIntelCache(cacheCollection *lpCaches, short code) {
    ...
    if (0xFF == code) {
        ...
        while(1) {
            ...
            ways = ((info[1] & 0xffc00000) >> 22) + 1;
            partitions = ((info[1] & 0x003ff000) >> 12) + 1;
            sets = (info[2] & 0xffffffff) + 1;
...
int discoverIntelCaches(cacheCollection* lpCaches) {
    ...
    if (1 == (info[EAX] & 0xff)) {
        for (i = 1; i < 4; i++) {
            if (0 == (info[i] & 0x80000000)) { // If MSB is set, this value is one among reserved
                if (info[i] & 0xff) {
