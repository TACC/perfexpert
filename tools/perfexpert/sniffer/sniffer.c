...
int testCounter(unsigned ctr)
{
	...
	int event_code;
	if (PAPI_event_name_to_code(event_list[ctr].PAPI_event_name, &event_code) == PAPI_OK && PAPI_add_event(event_set, event_code) == PAPI_OK)
	{
...
int is_derived(int event_code)
{
	...
	return strlen(info.derived) != 0 && strcmp(info.derived, "NOT_DERIVED") != 0 && strcmp(info.derived, "DERIVED_CMPD") != 0;
}
...
int attach_sampling_frequencies(int saved_sampling_freq[SAMPLING_CATEGORY_COUNT])
{
	...
	{
		if (PAPI_event_name_to_code(event_list[i].PAPI_event_name, &code) == PAPI_OK && (PAPI_query_event(code) == PAPI_OK))
		{
			if (!is_derived(code))
			{
				...
				if ((saved_sampling_freq[cat] = generate_prime(cat, saved_sampling_freq[cat])) <= 0)
				{
					fprintf (stderr, "Failed to generate a prime number within given range, exiting\n");
					return -1;
				}
...
int main(int argc, char* argv [])
{
...

	{
		...
		if (IS_EVENT_AVAILABLE(FML_INS) && IS_EVENT_AVAILABLE(FDV_INS))
		{
			...
			{
				USE_EVENT(FAD_INS);
				sprintf(num, "(%s + %s + %s)", event_list[FML_INS].PAPI_event_name, event_list[FDV_INS].PAPI_event_name, event_list[FAD_INS].PAPI_event_name);
			}
			else
			{
				sprintf(num, "(%s + %s)", event_list[FML_INS].PAPI_event_name, event_list[FDV_INS].PAPI_event_name);
			}
		}
		...
		else if (IS_EVENT_AVAILABLE(FP_COMP_OPS_EXE_SSE_FP_PACKED_DOUBLE) && IS_EVENT_AVAILABLE(FP_COMP_OPS_EXE_SSE_FP_SCALAR_SINGLE) && IS_EVENT_AVAILABLE(FP_COMP_OPS_EXE_SSE_PACKED_SINGLE) && IS_EVENT_AVAILABLE(FP_COMP_OPS_EXE_SSE_SCALAR_DOUBLE))
		{
			if (testCounter(FP_COMP_OPS_EXE_SSE_FP_SCALAR_SINGLE_SSE_SCALAR_DOUBLE) == 0)
			{
	...
	else if (IS_EVENT_AVAILABLE(SSEX_UOPS_RETIRED_PACKED_DOUBLE) && IS_EVENT_AVAILABLE(SSEX_UOPS_RETIRED_PACKED_SINGLE) && IS_EVENT_AVAILABLE(SSEX_UOPS_RETIRED_SCALAR_DOUBLE) && IS_EVENT_AVAILABLE(SSEX_UOPS_RETIRED_SCALAR_SINGLE))
	{
	...
	else if (IS_EVENT_AVAILABLE(SIMD_COMP_INST_RETIRED_PACKED_DOUBLE) && IS_EVENT_AVAILABLE(SIMD_COMP_INST_RETIRED_PACKED_SINGLE) && IS_EVENT_AVAILABLE(SIMD_COMP_INST_RETIRED_SCALAR_DOUBLE) && IS_EVENT_AVAILABLE(SIMD_COMP_INST_RETIRED_SCALAR_SINGLE))
	{
		// Do a dummy test to see if the single big counter can be added or do we have to break it down into multiple parts
	...
	else if (IS_EVENT_AVAILABLE(SIMD_FP_256_PACKED_SINGLE) && IS_EVENT_AVAILABLE(SIMD_FP_256_PACKED_DOUBLE) && IS_EVENT_AVAILABLE(FP_COMP_OPS_EXE_SSE_FP_PACKED_DOUBLE) && IS_EVENT_AVAILABLE(FP_COMP_OPS_EXE_SSE_FP_SCALAR_SINGLE) && IS_EVENT_AVAILABLE(FP_COMP_OPS_EXE_SSE_PACKED_SINGLE) && IS_EVENT_AVAILABLE(FP_COMP_OPS_EXE_SSE_SCALAR_DOUBLE))
	{
		if (testCounter(FP_COMP_OPS_EXE_SSE_FP_SCALAR_SINGLE_SSE_SCALAR_DOUBLE) == 0)
		{
			DOWNGRADE_EVENT(FP_COMP_OPS_EXE_SSE_FP_SCALAR);
...

	{
		...
		else	counter_err("instruction_accesses.L1i_hits");

		sprintf(instruction_accesses_L1i_hits, "instruction_accesses.L1i_hits = (%s * L1_ilat) / PAPI_TOT_INS\n", l1i_hits);
		...
		sprintf(instruction_accesses_overall, "instruction_accesses.overall = (%s * L1_ilat + %s * L2_lat + %s * mem_lat) / PAPI_TOT_INS\n", l1i_hits, lcpi_L2_ICA, lcpi_L2_ICM);
	}
	...
	else if (IS_EVENT_AVAILABLE(DTLB_LOAD_MISSES_CAUSES_A_WALK) && IS_EVENT_AVAILABLE(DTLB_STORE_MISSES_CAUSES_A_WALK))
	{
		USE_EVENT(DTLB_LOAD_MISSES_CAUSES_A_WALK);
		USE_EVENT(DTLB_STORE_MISSES_CAUSES_A_WALK);
		strcpy(data_TLB_overall, "data_TLB.overall = (DTLB_LOAD_MISSES:CAUSES_A_WALK + DTLB_STORE_MISSES:CAUSES_A_WALK) * TLB_lat / PAPI_TOT_INS\n");
	}
	...
	else if (IS_EVENT_AVAILABLE(FP_COMP_OPS_EXE_SSE_DOUBLE_PRECISION) && IS_EVENT_AVAILABLE(FP_COMP_OPS_EXE_SSE_FP) && IS_EVENT_AVAILABLE(FP_COMP_OPS_EXE_SSE_FP_PACKED) && IS_EVENT_AVAILABLE(FP_COMP_OPS_EXE_SSE_FP_SCALAR) && IS_EVENT_AVAILABLE(FP_COMP_OPS_EXE_SSE_SINGLE_PRECISION) && IS_EVENT_AVAILABLE(FP_COMP_OPS_EXE_X87))
	{
		// Do a dummy test to see if the single big counter can be added or do we have to break it down into multiple parts
	...
	do
	{
		...
		for (j=0; j<EVENT_COUNT; j++)
		{
			...
			if (IS_EVENT_USED(j) && j != TOT_INS)
			{
				PAPI_event_name_to_code(event_list[j].PAPI_event_name, &event_code);
				if (PAPI_add_event(event_set, event_code) == PAPI_OK)
				{
					...
					fprintf (fp, "--event %s:%d ", event_list[j].PAPI_event_name, event_list[j].sampling_freq);
					addCount++;
				}
