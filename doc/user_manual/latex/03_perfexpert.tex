...
\begin{verbatim}
perfexpert 0.1 --verbose=10 -modules=lcpi,hpctoolkit -p "ibrun" -- ./XSBench  -t 4 -s large
\end{verbatim}

Parameters can be passed to \texttt{ibrun} as follows:

\begin{verbatim}
perfexpert 0.1 --verbose=10 -modules=lcpi,hpctoolkit -p "ibrun -np 4" -- ./XSBench  -t 4 -s large
\end{verbatim}
...
\subsubsection{HPCToolkit}
 This is the simplest case. All you need to do is to specify which MPI launcher you want to use as a prefix in your PerfExpert command. For example:
	...
	\begin{verbatim}
	perfexpert 0.1 --modules=lcpi,hpctoolkit --module-option=lcpi,architecture=MIC  --module-option=hpctoolkit,mic-card=mic0 --module-option=hpctoolkit,prefix="ibrun.symm -m" -- ./lules2.0 -i 100 -s 8
	\end{verbatim}
