...
\frame{\frametitle{State of the art}
	\begin{itemize}
		...
		\item Performance events provide fairly accurate view of CPU execution
		\item Various tools exist to correlate perormance events to user code
		\item Examples: PerfExpert, TAU, HPCToolkit, VTune, Scalasca, etc.
	\end{itemize}
}

\frame{\frametitle{But memory is a bigger problem}
  \begin{columns}[T]
    \begin{column}{.5\textwidth}
     \begin{block}{}
	\includegraphics[width=\textwidth]{figures/cpu-mem-speed.eps}\\\small Source: Computer Architecture: A Quantitative Approach, 4th Edition by Hennessy and Patterson
% Your text here
    \end{block}
    \end{column}
    \begin{column}{.5\textwidth}
...
\frame{\frametitle{Performance counter info is limited}
	\begin{itemize}
		\item Performance events are too-fine grained for memory profiling
		\item Some performance events are ambiguous to interpret
		\item Performance event measurements cannot be scoped to just the important variables
	\end{itemize}
}

\frame{\frametitle{Fine-grain measurements}
	\begin{itemize}
		\item Performance events are measured on execution of each instruction
		\item \texttt{mov ah, 16} causes cache miss $\Rightarrow$ increment cache miss counter
...
\frame{\frametitle{Ambiguous interpretation}
	\begin{itemize}
...
\frame{\frametitle{Little or no scoping}
	\begin{itemize}
...
\frame{\frametitle{Hence, MACPO}
	\begin{itemize}
		\item \underline{M}emory \underline{A}ccess \underline{C}entric \underline{P}erformance \underline{O}ptimization
		\item Performance tool that analyzes memory access patterns to find sources of inefficiency
		\item Used as part of the compilation process
		\item Tracks accesses to arrays and structures within a function
		\item Tags each such access with source code location and other data
		\item Analyzes accesses to see if access patterns can be improved
		\item Works with C, C++ and Fortran code [+ Pthreads, OpenMP, MPI]
	\end{itemize}
}
...
\frame{\frametitle{How to use MACPO?}
	\begin{itemize}
		\item Compile application using \texttt{macpo.sh}
...
%\frame{\frametitle{Metric #1: Cycles per access}
	Example:\\
	\verb|Var "counts", seen 1073 times, estimated to cost 8.98 cycles on every access|
	\begin{itemize}
...
\frame{\frametitle{Sample application}
	\begin{itemize}
		...
		\item Source code online at: \texttt{cs.utexas.edu/users/ashay/xsede-2013/sample-programs.tar.gz}
		...
		\begin{itemize}
			...
			\item For each pair of random numbers, calculates \texttt{z}
			\item Checks a condition on \texttt{z}, based on the result increments a counter
		\end{itemize}
	\end{itemize}
}
...
\begin{frame}[fragile]
\frametitle{MACPO analysis output}
	\begin{verbatim}
macpo-analyze macpo.out
...
\frame{\frametitle{Problem resolution}
	\begin{itemize}
		\item MACPO shows cache thrashing for \texttt{counts} variable.
...
\begin{frame}[fragile]
\frametitle{MACPO analysis output for optimized code}
	\begin{verbatim}
macpo-analyze macpo.out
...
\frame{\frametitle{Review}
	\begin{itemize}
		\item Compiled application using \texttt{macpo.sh}
		\item Discovered cache thrashing for the \texttt{counts} array
		\item Padding the array reduced cache conflicts from ~70\% to 0\%
		\item Execution time improved from 9.14s to 3.17s (65\% improvement)
	\end{itemize}
}
